...
import com.jlkj.web.shopnew.enums.EnumDateType;
...
public class DateUtil {
	...
	**/
	...
	//(选择累计 今日 昨日 本周 筛选,开始日子,结束日子)
	public static DateDto getDayByType(String type, Date startDay, Date endDay){
		//Date date = DateUtil.getMongoDate();
		Date date = new Date();
		Date startAddDay = DateUtil.getFirstSecIntegralTime(date);
		Date endAddDay = DateUtil.getLastSecIntegralTime(date);
		switch (type){
			case ConstantDateType.ALL_DAY://默认累计 取一端
			case ConstantDateType.TODAY://今天
				endDay = DateUtil.getLastSecIntegralTime(date);
				startDay = DateUtil.getFirstSecIntegralTime(date);//某日期00:00:00

				endAddDay =  DateUtil.getLastSecIntegralTime(date);
				startAddDay = DateUtil.getFirstSecIntegralTime(date);
				break;
			case ConstantDateType.YESTERDAY://昨日
				startDay = DateUtil.getFirstSecIntegralTime(DateUtil.rollDay(date, -1));
				endDay = DateUtil.getLastSecIntegralTime(DateUtil.rollDay(date, -1));

				startAddDay = DateUtil.getFirstSecIntegralTime(DateUtil.rollDay(date, -1));
				endAddDay = DateUtil.getLastSecIntegralTime(DateUtil.rollDay(date, -1));
				break;
			case ConstantDateType.WEEK://本周
				startDay =  DateUtil.getFirstSecIntegralTime(DateUtil.getWeekStartDate());
				endDay = DateUtil.getLastSecIntegralTime(date);

				startAddDay = DateUtil.getFirstSecIntegralTime(DateUtil.getWeekStartDate());
				endAddDay = DateUtil.getLastSecIntegralTime(date);
				break;
			case ConstantDateType.MONTH://本月
				startDay = DateUtil.getFirstSecIntegralTime(DateUtil.getFirstDayOfMonth());
				endDay = DateUtil.getLastSecIntegralTime(date);

				startAddDay = DateUtil.getFirstSecIntegralTime(DateUtil.getFirstDayOfMonth());
				endAddDay = DateUtil.getLastSecIntegralTime(date);
				break;
			case ConstantDateType.THREE_MONTH://近三月
				endDay = DateUtil.getLastSecIntegralTime(date);
				startDay = DateUtil.getFirstSecIntegralTime(DateUtil.rollDay(endDay , -90));

				endAddDay = DateUtil.getLastSecIntegralTime(date);
				startAddDay = DateUtil.getFirstSecIntegralTime(DateUtil.rollDay(endDay , -90));

				break;
			case ConstantDateType.SELECT_DAY://筛选
				endDay = DateUtil.getLastSecIntegralTime(endDay);
				startDay = DateUtil.getFirstSecIntegralTime(startDay);//某日期00:00:00

				endAddDay = DateUtil.getLastSecIntegralTime(endDay);
				startAddDay = DateUtil.getFirstSecIntegralTime(startDay);
				break;
			default:
				break;
		}

		return DateDto.builder().startDay(startDay).endDay(endDay)
				.startAddDay(startAddDay).endAddDay(endAddDay).build();

	}


	public static DateDto getDayByType4Chart(String type, Date startDay, Date endDay){

		Date date = DateUtil.getMongoDate();
		Date startAddDay = DateUtil.getFirstSecIntegralTime(date);
		Date endAddDay = DateUtil.getLastSecIntegralTime(date);
		switch (type){
			case ConstantDateType.ALL_DAY:
			case ConstantDateType.TODAY://今天
				endDay =  DateUtil.getLastSecIntegralTime(date);
				startDay = DateUtil.getFirstSecIntegralTime(date);
				break;
			case ConstantDateType.YESTERDAY://昨日
				startDay = DateUtil.getFirstSecIntegralTime(DateUtil.rollDay(date, -1));
				endDay = DateUtil.getLastSecIntegralTime(date);
			...
			case ConstantDateType.THREE_MONTH:
				endDay = DateUtil.getLastSecIntegralTime(date);
				startDay = DateUtil.getFirstSecIntegralTime(DateUtil.rollDay(date , -90));
				break;
			case ConstantDateType.SELECT_DAY://筛选
				break;
			default:
